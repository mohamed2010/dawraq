...
export const passwordResetConfirmInput = z.object({ token: z.string().min(32).max(256), newPassword: password });
export const healthIntegrationConsentInput = z.object({ platform: z.enum(["apple_health", "health_connect"]), scopes: z.array(z.enum(["cycle_dates", "daily_symptoms", "medications"])).min(1).max(3).transform(scopes => [...new Set(scopes)].sort()) });
export const profileInput = z.object({ displayName: z.string().trim().min(1).max(80), averageCycleLength: z.number().int().min(20).max(45), typicalBleedingDays: z.number().int().min(1).max(14), relationshipStatus: z.enum(["single", "married"]), pregnancyStatus: z.enum(["not_pregnant", "pregnant", "not_sure"]), theme: z.enum(["light", "dark", "pink", "purple"]), language: z.enum(["ar", "en"]), fontScale: z.enum(["normal", "large", "extra"]).default("normal"), tryingToConceive: z.boolean().default(false), stealthMode: z.boolean(), onboardingCompleted: z.boolean() });
...
export const backupRestoreInput = z.object({
  ...
  backup: z.object({
    ...
    cycles: z.array(cycleInput).max(500),
    dailyEntries: z.array(dailyEntryInput).max(5000),
