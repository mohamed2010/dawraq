...
export const userProfiles = mysqlTable("user_profiles", {
  ...
  typicalBleedingDays: int("typical_bleeding_days").notNull().default(5),
  relationshipStatus: mysqlEnum("relationship_status", ["single", "married"]).notNull().default("single"),
  pregnancyStatus: mysqlEnum("pregnancy_status", ["not_pregnant", "pregnant", "not_sure"]).notNull().default("not_pregnant"),
...
export const dailyEntries = mysqlTable("daily_entries", {
  ...
  mood: mysqlEnum("mood", ["very_low", "low", "neutral", "good", "great", "irritable", "anxious"]).notNull(),
  painLevel: int("pain_level").notNull().default(0),
  symptomsJson: text("symptoms_json").notNull(),
  notes: text("notes"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
