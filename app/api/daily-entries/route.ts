...
export async function GET(request: Request) { try { const user = await getAuthenticatedUser(request); const entries = await listDailyEntriesForUser(user.id); return NextResponse.json(entries.map(entry => ({ ...entry, symptoms: parseJsonArray(entry.symptomsJson), customSymptoms: parseJsonArray(entry.customSymptomsJson) }))); } catch (error) { return routeError(error); } }
