...
export function AccountSecurityPanel({ email, onEmailChanged }: { email: string; onEmailChanged: () => Promise<unknown> }) {
  ...
  const submitEmail = async (event: FormEvent) => {
    event.preventDefault();
    try { await changeEmail.mutateAsync({ email: nextEmail, currentPassword: emailPassword }); await onEmailChanged(); setEmailPassword(""); toast.success("تم تعديل البريد الإلكتروني للحساب."); } catch (error) { toast.error(error instanceof Error ? error.message : "تعذر تعديل البريد الإلكتروني."); }
  };
  const submitPassword = async (event: FormEvent) => {
    event.preventDefault();
    if (newPassword.length < 8) return toast.error("يجب أن تتكون كلمة المرور الجديدة من 8 أحرف على الأقل.");
    try { await changePassword.mutateAsync({ currentPassword, newPassword }); setCurrentPassword(""); setNewPassword(""); toast.success("تم تغيير كلمة المرور. تظل جلستكِ الحالية صالحة."); } catch (error) { toast.error(error instanceof Error ? error.message : "تعذر تغيير كلمة المرور."); }
  };
  return <section className="surface-card page-card privacy-tools"><div className="section-header"><div><h2>بيانات الدخول للحساب</h2><p>يتطلب أي تعديل حساس كلمة المرور الحالية للحفاظ على أمان الحساب.</p></div><KeyRound size={21} color="var(--purple)" /></div><form className="settings-group form-stack" onSubmit={submitEmail}><h3>تعديل البريد الإلكتروني</h3><div className="field"><label htmlFor="account-email">البريد الجديد</label><input id="account-email" type="email" dir="ltr" value={nextEmail} onChange={event => setNextEmail(event.target.value)} autoComplete="email" required /></div><div className="field"><label htmlFor="email-current-password">كلمة المرور الحالية</label><input id="email-current-password" type="password" dir="ltr" value={emailPassword} onChange={event => setEmailPassword(event.target.value)} autoComplete="current-password" required /></div><button className="secondary-button" type="submit" disabled={changeEmail.isPending}>{changeEmail.isPending ? "جارٍ الحفظ..." : "حفظ البريد الجديد"}</button></form><form className="settings-group form-stack" onSubmit={submitPassword}><h3>تغيير كلمة المرور</h3><div className="field"><label htmlFor="current-password">كلمة المرور الحالية</label><input id="current-password" type="password" dir="ltr" value={currentPassword} onChange={event => setCurrentPassword(event.target.value)} autoComplete="current-password" required /></div><div className="field"><label htmlFor="new-password">كلمة المرور الجديدة</label><input id="new-password" type="password" dir="ltr" value={newPassword} onChange={event => setNewPassword(event.target.value)} autoComplete="new-password" minLength={8} required /><span className="field-hint">ثمانية أحرف على الأقل، ولا نعرض كلمة المرور أو نخزنها كنص واضح.</span></div><button className="secondary-button" type="submit" disabled={changePassword.isPending}>{changePassword.isPending ? "جارٍ التغيير..." : "تغيير كلمة المرور"}</button></form><div className="settings-group"><p>إذا نسيتِ كلمة المرور قبل تسجيل الدخول، سيحتاج التطبيق إلى إعداد خدمة إرسال بريد موثوقة لإرسال رابط استعادة مؤقت. لم نفعّل رسالة استعادة غير قابلة للتسليم أو رمزاً غير آمن.</p></div></section>;
}
