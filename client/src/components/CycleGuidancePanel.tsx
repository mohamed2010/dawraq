...
export function CycleGuidancePanel({ statistics, today, dailyEntry, tryingToConceive }: { statistics: CycleStatistics; today: string; dailyEntry: DailyEntry; tryingToConceive?: boolean }) {
  ...
  return <section className="surface-card page-card cycle-guidance-panel">
    ...
    {statistics.ovulationDate && statistics.fertileStart && statistics.fertileEnd ? <div className="fertility-summary"><div className="fertility-highlight"><Flower2 size={19} /><div><span>موعد التبويض المتوقع</span><strong>{formatDate(statistics.ovulationDate)}</strong></div></div><div><span>نافذة الخصوبة المتوقعة</span><strong>{formatDate(statistics.fertileStart)} — {formatDate(statistics.fertileEnd)}</strong></div><div><span>حالتكِ اليوم</span><strong>{phaseDetail.title}{statistics.daysAfterOvulation !== null ? ` · ${statistics.daysAfterOvulation} DPO` : ""}</strong></div></div> : <div className="empty-state">أضيفي على الأقل تاريخ بداية دورة لتظهر تقديرات التبويض والخصوبة الشخصية.</div>}
    {tryingToConceive && <div className="phase-note"><HeartHandshake size={16} /><span>وضع محاولة الحمل مفعّل: تابعي الحرارة الأساسية وملاحظات الخصوبة إن رغبتِ، لكن النتائج والتواريخ تقديرية وليست تأكيداً للتبويض أو الحمل.</span></div>}
