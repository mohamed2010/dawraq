...
export function ReportsAndBackupPanel() {
  ...
  const restoreBackup = api.backup.restore.useMutation();
  const [from, setFrom] = useState(daysAgo(90)); const [to, setTo] = useState(dateKey(new Date())); const [passphrase, setPassphrase] = useState(""); const [verifiedBackup, setVerifiedBackup] = useState(""); const [restorePassphrase, setRestorePassphrase] = useState(""); const [restoreData, setRestoreData] = useState<Pick<PersonalSummary, "profile" | "cycles" | "dailyEntries" | "medications"> | null>(null); const [restoreConfirmation, setRestoreConfirmation] = useState("");
  ...
  const verifyBackup = async (event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; try { const content = JSON.parse(await file.text()) as { format?: string; payload?: unknown }; setRestoreData(null); setVerifiedBackup(content.format === "zuhaira-encrypted-v1" && Array.isArray(content.payload) ? "تم التحقق من ملف النسخة المشفرة. أدخلي عبارة المرور لفكها داخل جهازكِ." : "هذا ليس ملف نسخة زُهيرة مشفرة صالحاً."); } catch { setVerifiedBackup("تعذر قراءة ملف النسخة."); } };
  const decryptBackup = async () => { try { const file = document.querySelector<HTMLInputElement>("#zuhaira-backup-file")?.files?.[0]; if (!file || restorePassphrase.length < 8) return toast.error("اختاري ملف النسخة واكتبي عبارة المرور الصحيحة."); const content = JSON.parse(await file.text()) as { salt: number[]; iv: number[]; payload: number[] }; const material = await crypto.subtle.importKey("raw", new TextEncoder().encode(restorePassphrase), "PBKDF2", false, ["deriveKey"]); const key = await crypto.subtle.deriveKey({ name: "PBKDF2", salt: new Uint8Array(content.salt), iterations: 150000, hash: "SHA-256" }, material, { name: "AES-GCM", length: 256 }, false, ["decrypt"]); const decrypted = await crypto.subtle.decrypt({ name: "AES-GCM", iv: new Uint8Array(content.iv) }, key, new Uint8Array(content.payload)); const backup = JSON.parse(new TextDecoder().decode(decrypted)) as PersonalSummary; setRestoreData({ profile: backup.profile, cycles: backup.cycles, dailyEntries: backup.dailyEntries, medications: backup.medications }); toast.success("تم فك النسخة محلياً. راجعي التأكيد قبل الاستعادة."); } catch { toast.error("تعذر فك النسخة. تحققي من عبارة المرور والملف."); } };
  const restore = async () => { if (!restoreData || restoreConfirmation !== "استعادة النسخة") return; try { await restoreBackup.mutateAsync({ confirmation: "استعادة النسخة", backup: restoreData }); toast.success("تمت استعادة النسخة. سيُعاد تحميل التطبيق الآن."); window.setTimeout(() => window.location.reload(), 700); } catch (error) { toast.error(error instanceof Error ? error.message : "تعذر استعادة النسخة."); } };
  return <section className="surface-card page-card enhancement-panel"><div className="section-header"><div><h2>تقارير ونسخ احتياطي</h2><p>اختاري فترة للتقرير، واحفظي نسخة مشفرة محلياً بعبارة مرور من اختياركِ.</p></div><FileText size={21} color="var(--accent)" /></div><div className="report-range"><div className="field"><label>من</label><input type="date" value={from} onChange={event => setFrom(event.target.value)} /></div><div className="field"><label>إلى</label><input type="date" value={to} onChange={event => setTo(event.target.value)} /></div></div><div className="day-actions"><button className="secondary-button" type="button" onClick={() => void load().then(data => printReport(data, from, to)).catch(error => toast.error(error instanceof Error ? error.message : "تعذر تجهيز التقرير."))}><FileText size={16} />تقرير للطباعة/PDF</button><button className="secondary-button" type="button" onClick={() => void load().then(data => saveDownload(`zuhaira-export-${dateKey(new Date())}.json`, JSON.stringify(filterSummary(data, from, to), null, 2), "application/json")).catch(() => toast.error("تعذر تنزيل البيانات."))}><Download size={16} />تصدير الفترة</button></div><div className="adherence-card"><strong>التزام الأدوية خلال آخر 30 يوماً</strong><span>{adherence?.rate === null ? "أضيفي دواءً نشطاً لبدء الحساب." : adherence ? `${adherence.rate}% — ${adherence.taken} جرعة مؤكدة من ${adherence.expected} مجدولة` : "حمّلي التقرير لإظهار الملخص."}</span></div><div className="settings-group"><h3>نسخة احتياطية مشفّرة</h3><p>تشفّر النسخة داخل جهازكِ قبل تنزيلها. لن يمكن فتحها بدون عبارة المرور، ولا تُرفع إلى خادم منفصل.</p><div className="field"><label>عبارة مرور النسخة</label><input type="password" minLength={8} value={passphrase} onChange={event => setPassphrase(event.target.value)} placeholder="8 أحرف أو أكثر" /></div><div className="day-actions"><button className="secondary-button" type="button" onClick={() => void encryptedBackup()}><KeyRound size={16} />تنزيل نسخة مشفرة</button></div></div><div className="settings-group"><h3>استعادة نسخة احتياطية</h3><p>الاستعادة تستبدل سجلات الدورة والمتابعة والأدوية في هذا الحساب فقط. لن تستعيد سجل الجرعات المؤكدة.</p><label className="secondary-button upload-label"><Upload size={16} />اختيار ملف النسخة<input id="zuhaira-backup-file" type="file" accept="application/json" onChange={verifyBackup} /></label>{verifiedBackup && <p className="field-hint">{verifiedBackup}</p>}<div className="field"><label>عبارة مرور الاستعادة</label><input type="password" minLength={8} value={restorePassphrase} onChange={event => setRestorePassphrase(event.target.value)} placeholder="عبارة مرور النسخة" /></div><button className="secondary-button" type="button" onClick={() => void decryptBackup()}><KeyRound size={16} />فك النسخة محلياً</button>{restoreData && <><div className="field"><label>للتأكيد، اكتبي: استعادة النسخة</label><input value={restoreConfirmation} onChange={event => setRestoreConfirmation(event.target.value)} placeholder="استعادة النسخة" /></div><button className="danger-button" type="button" disabled={restoreConfirmation !== "استعادة النسخة" || restoreBackup.isPending} onClick={() => void restore()}>{restoreBackup.isPending ? "جارٍ الاستعادة..." : "استبدال بياناتي بالنسخة"}</button></>}</div><p className="field-hint"><ShieldCheck size={14} />لا تستوردي ملفاً من شخص آخر، ولا تشاركي عبارة المرور مع أي شخص.</p></section>;
}
